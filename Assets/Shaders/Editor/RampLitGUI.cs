    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            EditorGUILayout.BeginVertical();
            {
                EditorGUILayout.BeginHorizontal();
                {
                    _RimColor.colorValue = EditorGUILayout.ColorField(_RimColor.colorValue, GUILayout.Width(70));
        ...
        /*_ReceiveShadows.floatValue = result ? 1 : 0;

        if (result)
        {
            materialEditor.DefaultShaderProperty(_ShadowColor, _ShadowColor.displayName);
        }*/

        //materialEditor.DefaultShaderProperty(_ReceiveShadows, _ReceiveShadows.displayName);

        /*for (int i = 0; i < properties.Length; i++)
        {
            materialEditor.DefaultShaderProperty(properties[i], properties[i].name);
        }*/
